import { useParametersForEvaluation } from '@/services/queries'; // Import your parameter queries
import Parameter from '@/types/parameters'; // Assuming you have a type for Parameter
...
export const useAddParameter = (evalId: string) => { // Take evalId as input
...
export const useUpdateParameter = (evalId: string) => { // Take evalId as input
  const { mutate } = useParametersForEvaluation(evalId);
  const [isLoading, setIsLoading] = useState(false);
...
export const useDeleteParameter = (evalId: string) => { // Take evalId as input
  const { mutate } = useParametersForEvaluation(evalId);
  const [isLoading, setIsLoading] = useState(false);
  ...
  const deleteParameter = useCallback(
    async (parameterId: string) => {
      ...
      mutate((currentData) => {
        ...
        return {
          ...currentData,
          data: currentData.data.filter((parameter) => parameter.id !== parameterId),
        };
      }, false);
